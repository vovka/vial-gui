...
from widgets.combo_visualization import ComboRenderer, ComboSpec, KeyInfo, Point
...
class KeyboardWidget(QWidget):
    ...
    def __init__(self, layout_editor):
        ...
        self.combo_renderer = ComboRenderer()
    ...
    def _draw_combos(self, qp):
        combo_specs = self._build_combo_specs()
        if combo_specs:
            self.combo_renderer.render(qp, combo_specs, self.scale)

    def _build_combo_specs(self):
        """Convert collected combo widgets to ComboSpec objects."""
        combos = self._collect_combo_widgets()
        specs = []
        for widgets, output_label, combo_label in combos:
            keys = []
            for widget in widgets:
                bbox = widget.polygon.boundingRect()
                center = bbox.center()
                keys.append(KeyInfo(
            ...
            specs.append(ComboSpec(keys, output_label, combo_label))
        return specs
