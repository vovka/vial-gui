...
class KeyboardWidget(QWidget):
    ...
    def _compute_keyboard_center(self, key_rects):
        """Compute the center of the keyboard layout for split detection."""
        if not key_rects:
            return 0
        min_x = min(r.left() for r in key_rects)
        max_x = max(r.right() for r in key_rects)
        return (min_x + max_x) / 2

    def _detect_split_gap(self, key_rects, keyboard_center):
        """Detect if there's a gap in the middle (split keyboard) and return gap bounds."""
        if not key_rects:
            return None

        left_keys = [r for r in key_rects if r.center().x() < keyboard_center]
        right_keys = [r for r in key_rects if r.center().x() >= keyboard_center]
        ...
        avg_key_width = sum(r.width() for r in key_rects) / len(key_rects)
        if right_edge - left_edge > avg_key_width * 0.5:
            return (left_edge, right_edge)
        return None

    def _compute_directional_preference(self, combo_widgets, key_rects, canvas_width, canvas_height):
        """
        Compute the preferred direction for label placement based on key positions.
        Returns a list of directions sorted by preference: 'right', 'left', 'above', 'below'
        """
        ...
        keyboard_center_x = canvas_width / 2
        keyboard_center_y = canvas_height / 2

        all_key_centers = [r.center() for r in key_rects]
        if all_key_centers:
            keyboard_center_x = sum(c.x() for c in all_key_centers) / len(all_key_centers)
            keyboard_center_y = sum(c.y() for c in all_key_centers) / len(all_key_centers)

        dx = avg_x - keyboard_center_x
        dy = avg_y - keyboard_center_y
        ...
        if abs(dx) > abs(dy):
            if dx > 0:
                ...
                directions = ['left', 'above', 'below', 'right']
        else:
            if dy > 0:
                directions = ['below', 'right', 'left', 'above']
            else:
                directions = ['above', 'right', 'left', 'below']

        return directions

    def _score_label_position(self, rect, combo_widgets, key_rects, placed_rects, placed_lines,
                              canvas_width, canvas_height, split_gap):
        """
        Score a candidate label position. Lower score is better.
        Considers: overlaps, line crossings, edge proximity, aesthetic placement.
        """
        score = 0
        ...
        margin = 5
        if rect.left() < margin or rect.right() > canvas_width - margin:
            score += 50
        if rect.top() < margin or rect.bottom() > canvas_height - margin:
            score += 50

        rect_center = rect.center()
        combo_key_rects = [w.polygon.boundingRect() for w in combo_widgets]
        ...
        for widget in combo_widgets:
            ...
            for key_rect in key_rects:
                if key_rect in combo_key_rects:
                    continue
                if self._line_crosses_rect(rect_center, key_center, key_rect):
                    score += 30

        for prev_label_center, prev_key_centers in placed_lines:
            for key_center in [w.polygon.boundingRect().center() for w in combo_widgets]:
                for prev_key_center in prev_key_centers:
                    if self._lines_intersect(rect_center, key_center, prev_label_center, prev_key_center):
                        score += 20

        if split_gap:
            gap_center = (split_gap[0] + split_gap[1]) / 2
            if split_gap[0] < rect.center().x() < split_gap[1]:
                score -= 10

        return score

    def _line_crosses_rect(self, p1, p2, rect):
        """Check if line from p1 to p2 crosses through a rectangle (not just touches edge)."""
        shrunk = rect.adjusted(rect.width() * 0.15, rect.height() * 0.15,
                               -rect.width() * 0.15, -rect.height() * 0.15)

        edges = [
            (QPointF(shrunk.left(), shrunk.top()), QPointF(shrunk.right(), shrunk.top())),
            (QPointF(shrunk.right(), shrunk.top()), QPointF(shrunk.right(), shrunk.bottom())),
            (QPointF(shrunk.right(), shrunk.bottom()), QPointF(shrunk.left(), shrunk.bottom())),
            (QPointF(shrunk.left(), shrunk.bottom()), QPointF(shrunk.left(), shrunk.top())),
        ]
    ...
    def _lines_intersect(self, a1, a2, b1, b2):
        ...
        return self._segments_intersect(a1, a2, b1, b2)

    def _find_line_rect_intersections(self, p1, p2, rect):
        """Find intersection points of a line with rectangle edges."""
        intersections = []
        edges = [
        ...
        for e1, e2 in edges:
            intersection = self._line_intersection(p1, p2, e1, e2)
            if intersection:
    ...
    def _line_intersection(self, p1, p2, p3, p4):
        """Find intersection point of two line segments, or None if they don't intersect."""
        x1, y1 = p1.x(), p1.y()
        x2, y2 = p2.x(), p2.y()
        x3, y3 = p3.x(), p3.y()
        x4, y4 = p4.x(), p4.y()
    ...
    def _compute_curved_path(self, start, end, obstacles, combo_key_rects, avg_size):
        """
        Compute a curved path from start to end that avoids obstacles.
        Returns a QPainterPath with smooth curves.
        """
        path = QPainterPath()
        path.moveTo(start)
        ...
        for rect in obstacles:
            if rect in combo_key_rects:
                continue
            if self._line_crosses_rect(start, end, rect):
                blocking_rects.append(rect)

        if not blocking_rects:
            dx = end.x() - start.x()
            dy = end.y() - start.y()
            ...
            if dist > avg_size * 2:
                ...
                perp_x = -dy / dist * avg_size * 0.15
                perp_y = dx / dist * avg_size * 0.15
        ...
        blocking_rects.sort(key=lambda r: math.hypot(
            r.center().x() - start.x(), r.center().y() - start.y()))
        ...
        for rect in blocking_rects:
            ...
            margin = avg_size * 0.3
            if cross > 0:
                waypoint = QPointF(rect.left() - margin, rect.center().y())
            else:
                waypoint = QPointF(rect.right() + margin, rect.center().y())

            if abs(dy) > abs(dx):
                if cross > 0:
                    waypoint = QPointF(rect.center().x(), rect.top() - margin)
                else:
                    waypoint = QPointF(rect.center().x(), rect.bottom() + margin)

            ctrl1 = QPointF((current.x() + waypoint.x()) / 2, current.y())
            ctrl2 = QPointF(waypoint.x(), (current.y() + waypoint.y()) / 2)
    ...
    def _draw_combos(self, qp):
        ...
        keyboard_center = self._compute_keyboard_center(key_rects)
        split_gap = self._detect_split_gap(key_rects, keyboard_center)
        ...
        for combo_widgets, output_label, combo_label in combos:
            ...
            for widget in combo_widgets:
                ...
                centers.append(widget_bbox.center())
            ...
            directions = self._compute_directional_preference(combo_widgets, key_rects,
            ...
            for direction in directions:
                if direction == 'above':
                    candidates.append(QRectF(bbox.center().x() - rect_w / 2,
                                            bbox.top() - gap - rect_h, rect_w, rect_h))
                elif direction == 'below':
                    candidates.append(QRectF(bbox.center().x() - rect_w / 2,
                                            bbox.bottom() + gap, rect_w, rect_h))
                elif direction == 'left':
                    candidates.append(QRectF(bbox.left() - gap - rect_w,
                                            bbox.center().y() - rect_h / 2, rect_w, rect_h))
                elif direction == 'right':
                    candidates.append(QRectF(bbox.right() + gap,
                                            bbox.center().y() - rect_h / 2, rect_w, rect_h))
            ...
            if split_gap:
                gap_center_x = (split_gap[0] + split_gap[1]) / 2
                candidates.append(QRectF(gap_center_x - rect_w / 2, bbox.top(), rect_w, rect_h))
                candidates.append(QRectF(gap_center_x - rect_w / 2, bbox.bottom() - rect_h, rect_w, rect_h))
                candidates.append(QRectF(gap_center_x - rect_w / 2, bbox.center().y() - rect_h / 2, rect_w, rect_h))

            best_rect = None
            best_score = float('inf')

            for candidate in candidates:
                clamped = clamp_rect(candidate)
                score = self._score_label_position(clamped, combo_widgets, key_rects,
                                                   placed_rects, placed_lines,
                                                   canvas_width, canvas_height, split_gap)
                if score < best_score:
                    best_score = score
                    best_rect = clamped
            ...
            if best_rect is None or best_score >= 1000:
                fallback = QRectF(bbox.center().x() - rect_w / 2,
                                 bbox.top() - gap - rect_h, rect_w, rect_h)
                fallback = clamp_rect(fallback)
                step_y = rect_h + gap
                attempts = 0
                while self._score_label_position(fallback, combo_widgets, key_rects,
                                                  placed_rects, placed_lines,
                                                  canvas_width, canvas_height, split_gap) >= 500 and attempts < 8:
                    fallback = clamp_rect(QRectF(fallback.x(), fallback.y() + step_y, rect_w, rect_h))
                    attempts += 1
                best_rect = fallback

            placed_rects.append(best_rect)
            key_centers = [w.polygon.boundingRect().center() for w in combo_widgets]
            placed_lines.append((best_rect.center(), key_centers))

            combo_placements.append((combo_widgets, output_label, combo_label, best_rect,
                                     avg_size, adjacent, name_height, label_height, text_gap))

        for (combo_widgets, output_label, combo_label, rect, avg_size, adjacent,
             name_height, label_height, text_gap) in combo_placements:
            ...
            if not adjacent:
                ...
                for widget in combo_widgets:
                    key_center = widget.polygon.boundingRect().center()
                    path = self._compute_curved_path(rect_center, key_center,
                                                     key_rects, combo_key_rects, avg_size)
                    qp.drawPath(path)
